...
const Group = ({ title, currentPage, navItems }) => (
  <Collapsible.Root defaultOpen={currentPage?.indexOf(title) === 0}>
    <Collapsible.Trigger
      ...
      <Collapsible.Content className="relative">
        ...
        <div className="py-2 flex flex-col truncate">
          <LayoutGroup id="sidebar">
            {navItems &&
              navItems.filter(item => item.indexOf(title) === 0).map((item) => {
                    ...
                    <motion.span
                      className={classnames(
                        'text-[14px] flex items-center font-medium gap-2 w-full pl-4 h-8 rounded-md text-slate-11 relative transition ease-in-out duration-200',
                        {
                          ...
                          'hover:text-slate-12': !isCurrentPage,
                        },
                      )}
                    >
                      {isCurrentPage && (
